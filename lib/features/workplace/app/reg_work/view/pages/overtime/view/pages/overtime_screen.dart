...
class OvertimeScreenPage extends StatefulWidget {
  const OvertimeScreenPage({super.key});

  @override
  State<OvertimeScreenPage> createState() => _OvertimeScreenPageState();
}

class _OvertimeScreenPageState
    extends BaseState<OvertimeScreenPage, OvertimeEvent, OvertimeState, OvertimeBloc> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      bloc.add(const OvertimeEvent.init());
    });
  }

  Future<void> _openOvertimeDetail(
    BuildContext context,
    OvertimeItem item,
  ) async {
    final reload = await context.push<bool?>('/regwork/overtime/detail', extra: item);
    if (!mounted) return;
    if (reload == true) {
      bloc.add(const OvertimeEvent.init());
    }
  ...
  Widget _buildBody(BuildContext context, OvertimeState state) {
    final items = state.overtime;
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            FilledButton(
              onPressed: () => bloc.add(const OvertimeEvent.init()),
    ...
    final grouped = <DateTime, List<OvertimeItem>>{};
    ...
    for (final day in sortedDays) {
      for (final item in grouped[day]!) {
        final canSwipe = OvertimeCard.canDelete(item);

        if (!canSwipe) {
          listWidgets.add(
            OvertimeCard(
              item: item,
              onTap: () => _openOvertimeDetail(context, item),
            ),
          );
          continue;
        }
        ...
        listWidgets.add(
          Slidable(
            key: ValueKey('overtime_${item.id}'),
            groupTag: 'overtime_slidable',
              ...
              children: [
                SlidableAction(
                  onPressed: (actionContext) async {
                    ...
                    if (!mounted) return;
                    if (confirmed) {
                      bloc.add(OvertimeEvent.onCancelSubmit(id: item.id));
                    }
                  },
            ...
            child: Builder(
              builder: (slidableCtx) => OvertimeCard(
                item: item,
                onTap: () {
                  Slidable.of(slidableCtx)?.close();
                  _openOvertimeDetail(context, item);
                },
              ),
            ),
          ),
        ...
        builder: (context, state) {
          return BaseScaffold(
            ...
            floatingActionButton: FloatingActionButton(
              onPressed: () async {
                final reload =
                    await context.push<bool?>('/regwork/overtime/add');
                if (!mounted) return;
                if (reload == true) {
                  bloc.add(const OvertimeEvent.init());
                }
