...
/// Card hiển thị một phiếu đặt cơm trong danh sách với style glassmorphism.
class LunchCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final employee = (item.fullName ?? '').trim().isNotEmpty
        ? '${(item.code ?? '').trim().isNotEmpty ? '${item.code} - ' : ''}${item.fullName}'
        : (item.code ?? 'Nhân viên');
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _InfoRow(
                              ...
                              label: 'Ngày',
                              value: _formatDate(item.dateOrder),
                            ),
                            const SizedBox(height: 8),
                            _InfoRow(
