...
import '../widgets/work_trip_card.dart';

class WorkTripScreenPage extends StatefulWidget {
  const WorkTripScreenPage({super.key});
...
class _WorkTripScreenPageState
  ...
  Future<void> _openDetail(BuildContext context, WorkTripItem item) async {
    final reload =
        await context.push<bool?>('/regwork/work_trip/detail', extra: item);
    if (!mounted) return;
    if (reload == true) {
      bloc.add(const WorkTripEvent.init());
    }
  ...
  Widget _buildBody(BuildContext context, WorkTripState state) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            FilledButton(
              onPressed: () => bloc.add(const WorkTripEvent.init()),
    ...
    final grouped = <DateTime, List<WorkTripItem>>{};
    for (final item in items) {
      final day = _dateOnly(item.dayBussiness ?? DateTime.now());
    ...
    for (final day in sortedDays) {
      for (final item in grouped[day]!) {
        final canSwipe = WorkTripCard.canDelete(item);
        ...
        listWidgets.add(
          Slidable(
            key: ValueKey('work_trip_${item.id}'),
            groupTag: 'work_trip_slidable',
              ...
              children: [
                SlidableAction(
                  onPressed: (actionContext) async {
                    ...
                    if (!mounted) return;
                    if (confirmed) {
                      bloc.add(WorkTripEvent.onCancelSubmit(id: item.id));
                    }
                  },
            ...
            child: Builder(
              builder: (slidableCtx) => WorkTripCard(
                item: item,
                onTap: () {
        ...
        builder: (context, state) {
          return Stack(
            ...
            floatingActionButton: FloatingActionButton(
              onPressed: () async {
                final reload =
                    await context.push<bool?>('/regwork/work_trip/add');
                if (!mounted) return;
                if (reload == true) {
                  bloc.add(const WorkTripEvent.init());
                }
