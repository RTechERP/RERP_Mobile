...
import '../../view/bloc/summary_overnight_bloc.dart';
import '../../view/widgets/summary_overnight_card.dart';
import '../../view/widgets/summary_overnight_filter_sheet.dart';

/// Trang tổng hợp phiếu làm đêm. Cho phép lọc theo ngày, phòng ban,
/// trạng thái và tìm kiếm theo từ khoá. Khi chưa chọn phòng ban, danh sách
/// được nhóm theo phòng ban; khi đã chọn phòng ban, hiển thị phẳng.
class SummaryOvernightScreen extends StatefulWidget {
  const SummaryOvernightScreen({super.key});

  @override
  State<SummaryOvernightScreen> createState() => _SummaryOvernightScreenState();
}

class _SummaryOvernightScreenState extends BaseState<
    SummaryOvernightScreen,
    SummaryOvernightEvent,
    SummaryOvernightState,
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      bloc.add(const SummaryOvernightEvent.init());
    });
  }
    ...
    setState(() {
      ...
      if (_isSearchOpen) {
        ...
        if (_searchController.text.isNotEmpty) {
          ...
          bloc.add(const SummaryOvernightEvent.changeKeyword(keyword: ''));
        }
      }
    });
  }
  ...
  void _showDateRangePicker(BuildContext context, SummaryOvernightState state) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => DateRangePicker(
        ...
        onApply: (start, end) {
          bloc.add(SummaryOvernightEvent.changeDateRange(
            dateStart: start,
            dateEnd: end,
          ));
        },
  ...
  void _showFilterSheet(BuildContext context, SummaryOvernightState state) {
    showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (sheetContext) => SummaryOvernightFilterSheet(
        departments: state.departments,
        selectedDepartmentId: state.departmentId,
        selectedStatus: state.filterStatus,
        onApply: (departmentId, status) {
          bloc.add(SummaryOvernightEvent.changeFilters(
            departmentId: departmentId,
            status: status,
          ));
        },
        onClear: () {
          bloc.add(const SummaryOvernightEvent.changeFilters(
            departmentId: null,
            status: null,
          ));
        },
  ...
  void _onSearchChanged(String value) {
    bloc.add(SummaryOvernightEvent.changeKeyword(keyword: value));
  }

  void _onSearchSubmitted(String value) {
    bloc.add(SummaryOvernightEvent.changeKeyword(keyword: value.trim()));
  }

  @override
  Widget renderUI(BuildContext context) {
    return BlocBuilder<SummaryOvernightBloc, SummaryOvernightState>(
        ...
        return BaseScaffold(
          ...
          body: Column(
            children: [
              ...
              if (state.filterStatus != null) _buildActiveFilters(state),
              Expanded(
                child: _buildContent(
                  context,
                  state,
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
              borderSide:
                  const BorderSide(color: AppColors.primaryERP, width: 1.5),
            ),
          ),
        ),
      ),
  ...
  Widget _buildActiveFilters(SummaryOvernightState state) {
        ...
        children: [
          ...
          if (state.filterStatus != null)
            Chip(
              ...
              backgroundColor:
                  _getStatusColor(state.filterStatus).withValues(alpha: 0.1),
              side: BorderSide(color: _getStatusColor(state.filterStatus)),
              labelStyle:
                  TextStyle(color: _getStatusColor(state.filterStatus)),
  ...
  Map<String, List<SummaryOvernightItem>> _groupByDepartment(
    List<SummaryOvernightItem> items,
    List<SummaryOvernightDepartment> departments,
  ) {
    final grouped = <String, List<SummaryOvernightItem>>{};
    ...
    final sorted = Map<String, List<SummaryOvernightItem>>.fromEntries(
  ...
  int _countDistinctEmployees(List<SummaryOvernightItem> items) {
    final seen = <int>{};
    for (final item in items) {
  ...
  Widget _buildContent(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              FilledButton(
                onPressed: () => bloc.add(const SummaryOvernightEvent.refresh()),
    ...
    if (state.departmentId != null) {
      return RefreshIndicator(
        onRefresh: () async {
          bloc.add(const SummaryOvernightEvent.refresh());
          await bloc.stream
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) =>
              SummaryOvernightCard(item: items[index]),
          ...
          return Column(
            ...
            children: [
              ...
              ...deptItems.map((item) => Padding(
                    ...
                    child: SummaryOvernightCard(item: item),
                  )),
            ],
          );
        },
              ...
              child: Row(
                ...
                children: [
                  Icon(Icons.event_outlined,
                      size: 14, color: AppColors.primaryERP),
