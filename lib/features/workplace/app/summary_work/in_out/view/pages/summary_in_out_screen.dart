...
import '../../view/bloc/summary_in_out_bloc.dart';
import '../../view/widgets/summary_in_out_card.dart';
import '../../view/widgets/summary_in_out_filter_sheet.dart';

class SummaryInOutScreen extends StatefulWidget {
  const SummaryInOutScreen({super.key});

  @override
  State<SummaryInOutScreen> createState() => _SummaryInOutScreenState();
}

class _SummaryInOutScreenState extends BaseState<
    SummaryInOutScreen,
    SummaryInOutEvent,
    SummaryInOutState,
    SummaryInOutBloc> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      bloc.add(const SummaryInOutEvent.init());
    });
  }
    ...
    setState(() {
      ...
      if (_isSearchOpen) {
        ...
        if (_searchController.text.isNotEmpty) {
          ...
          bloc.add(const SummaryInOutEvent.changeKeyword(keyword: ''));
        }
      }
    });
  }
  ...
  void _showDateRangePicker(BuildContext context, SummaryInOutState state) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => DateRangePicker(
        ...
        onApply: (start, end) {
          bloc.add(SummaryInOutEvent.changeDateRange(
            dateStart: start,
            dateEnd: end,
          ));
        },
  ...
  void _showFilterSheet(BuildContext context, SummaryInOutState state) {
    showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (sheetContext) => SummaryInOutFilterSheet(
        departments: state.departments,
        selectedDepartmentId: state.departmentId,
        selectedStatus: state.filterStatus,
        onApply: (departmentId, status) {
          bloc.add(SummaryInOutEvent.changeFilters(
            departmentId: departmentId,
            status: status,
          ));
        },
        onClear: () {
          bloc.add(const SummaryInOutEvent.changeFilters(
            departmentId: null,
            status: null,
          ));
        },
  ...
  void _onSearchChanged(String value) {
    bloc.add(SummaryInOutEvent.changeKeyword(keyword: value));
  }

  void _onSearchSubmitted(String value) {
    bloc.add(SummaryInOutEvent.changeKeyword(keyword: value.trim()));
  }

  Map<String, List<SummaryInOutItem>> _groupByDepartment(
    List<SummaryInOutItem> items,
    List<SummaryInOutDepartment> departments,
  ) {
    final grouped = <String, List<SummaryInOutItem>>{};
    ...
    final sorted = Map<String, List<SummaryInOutItem>>.fromEntries(
  ...
  int _countDistinctEmployees(List<SummaryInOutItem> items) {
    final seen = <int>{};
    for (final item in items) {
  ...
  Widget renderUI(BuildContext context) {
    return BlocBuilder<SummaryInOutBloc, SummaryInOutState>(
          ...
          appBar: AppBarCommon(
            ...
            actions: [
              ...
              IconButton(
                icon: Badge(
                  isLabelVisible:
                      state.departmentId != null || state.filterStatus != null,
  ...
  Widget _buildActiveFilters(SummaryInOutState state) {
  ...
  Widget _buildContent(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            FilledButton(
              onPressed: () => bloc.add(const SummaryInOutEvent.refresh()),
    ...
    if (state.departmentId != null) {
      return RefreshIndicator(
        onRefresh: () async {
          bloc.add(const SummaryInOutEvent.refresh());
          await bloc.stream
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) => SummaryInOutCard(item: items[index]),
          ...
          return Column(
            ...
            children: [
              ...
              ...deptItems.map((item) => Padding(
                    ...
                    child: SummaryInOutCard(item: item),
                  )),
            ],
          );
        },
