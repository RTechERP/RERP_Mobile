...
/// Card hiển thị một đơn đi muộn/về sớm trong danh sách summary.
class SummaryInOutCard extends StatelessWidget {
  const SummaryInOutCard({super.key, required this.item});

  final SummaryInOutItem item;
  ...
  String _timeRegisterLine() {
    if (item.timeRegister == null) return '-';
    return '${item.timeRegister!.toStringAsFixed(0)} phút';
  }
                ...
                Row(
                  children: [
                    Expanded(
                      ...
                      child: _TypeChip(
                        label: 'Loại',
                        value: item.typeText ?? 'Đi muộn/về sớm',
