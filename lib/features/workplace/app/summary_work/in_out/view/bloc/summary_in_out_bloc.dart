...
import '../../data/repository/summary_in_out_repo.dart';

part 'summary_in_out_event.dart';
part 'summary_in_out_state.dart';
part 'summary_in_out_bloc.g.dart';
part 'summary_in_out_bloc.freezed.dart';

@injectable
class SummaryInOutBloc
    extends BaseBloc<SummaryInOutEvent, SummaryInOutState> {
  final SummaryInOutRepo _repo;
  ...
  SummaryInOutBloc(
    ...
    this._authRepo,
    this._log,
  ) : super(SummaryInOutState.init()) {
    on<SummaryInOutEvent>((event, emit) async {
      await event.when(
        init: () => _onInit(emit),
  ...
  Future<void> _onInit(Emitter<SummaryInOutState> emit) async {
  ...
  Future<void> _onChangeDateRange(
    Emitter<SummaryInOutState> emit, {
  ...
  Future<void> _loadInOut(
    Emitter<SummaryInOutState> emit, {
    required DateTime start,
    required DateTime end,
    required List<SummaryInOutDepartment> departments,
    int? departmentId,
  }) async {
    ...
    _log.logI('SummaryInOut payload: $payload');

    final res = await _repo.getInOut(payload: payload);
    await res.fold(
      (l) async {
        _log.logE('Get in_out failed: $l');
        emit(state.copyWith(
      ...
      (r) async {
        _log.logI('SummaryInOut success - total: ${r.length}');
        emit(
          state.copyWith(
