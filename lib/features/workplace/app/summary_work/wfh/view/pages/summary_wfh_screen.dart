...
import '../../view/bloc/summary_wfh_bloc.dart';
import '../../view/widgets/summary_wfh_card.dart';
import '../../view/widgets/summary_wfh_filter_sheet.dart';

/// Trang tổng hợp đơn WFH. Cho phép lọc theo ngày, phòng ban, trạng thái
/// và tìm kiếm theo từ khoá. Khi chưa chọn phòng ban, danh sách được nhóm
/// theo phòng ban; khi đã chọn phòng ban, hiển thị phẳng.
class SummaryWfhScreen extends StatefulWidget {
  const SummaryWfhScreen({super.key});

  @override
  State<SummaryWfhScreen> createState() => _SummaryWfhScreenState();
}

class _SummaryWfhScreenState
    extends BaseState<SummaryWfhScreen, SummaryWfhEvent, SummaryWfhState, SummaryWfhBloc> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      bloc.add(const SummaryWfhEvent.init());
    });
  }
    ...
    setState(() {
      ...
      if (_isSearchOpen) {
        ...
        if (_searchController.text.isNotEmpty) {
          ...
          bloc.add(const SummaryWfhEvent.changeKeyword(keyword: ''));
        }
      }
    });
  }
  ...
  void _showDateRangePicker(BuildContext context, SummaryWfhState state) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => DateRangePicker(
        ...
        onApply: (start, end) {
          bloc.add(SummaryWfhEvent.changeDateRange(
            dateStart: start,
            dateEnd: end,
          ));
        },
  ...
  void _showFilterSheet(BuildContext context, SummaryWfhState state) {
    showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (sheetContext) => SummaryWfhFilterSheet(
        departments: state.departments,
        selectedDepartmentId: state.departmentId,
        selectedStatus: state.filterStatus,
        onApply: (departmentId, status) {
          bloc.add(SummaryWfhEvent.changeFilters(
            departmentId: departmentId,
            status: status,
          ));
        },
        onClear: () {
          bloc.add(const SummaryWfhEvent.changeFilters(
            departmentId: null,
            status: null,
          ));
        },
  ...
  void _onSearchChanged(String value) {
    bloc.add(SummaryWfhEvent.changeKeyword(keyword: value));
  }

  void _onSearchSubmitted(String value) {
    bloc.add(SummaryWfhEvent.changeKeyword(keyword: value.trim()));
  }

  @override
  Widget renderUI(BuildContext context) {
    return BlocBuilder<SummaryWfhBloc, SummaryWfhState>(
  ...
  Widget _buildActiveFilters(SummaryWfhState state) {
  ...
  Map<String, List<SummaryWfhItem>> _groupByDepartment(
    List<SummaryWfhItem> items,
    List<SummaryWfhDepartment> departments,
  ) {
    final grouped = <String, List<SummaryWfhItem>>{};
    ...
    final sorted = Map<String, List<SummaryWfhItem>>.fromEntries(
  ...
  int _countDistinctEmployees(List<SummaryWfhItem> items) {
    final seen = <int>{};
    for (final item in items) {
  ...
  Widget _buildContent(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              FilledButton(
                onPressed: () => bloc.add(const SummaryWfhEvent.refresh()),
    ...
    if (state.departmentId != null) {
      return RefreshIndicator(
        onRefresh: () async {
          bloc.add(const SummaryWfhEvent.refresh());
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) => SummaryWfhCard(item: items[index]),
          ...
          return Column(
            ...
            children: [
              ...
              ...deptItems.map((item) => Padding(
                    ...
                    child: SummaryWfhCard(item: item),
                  )),
            ],
          );
        },
