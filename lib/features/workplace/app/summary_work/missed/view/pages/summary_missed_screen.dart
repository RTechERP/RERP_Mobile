...
import '../../view/bloc/summary_missed_bloc.dart';
import '../../view/widgets/summary_missed_card.dart';
import '../../view/widgets/summary_missed_filter_sheet.dart';

/// Trang tổng hợp phiếu quên chấm công. Cho phép lọc theo ngày, phòng ban,
...
class SummaryMissedScreen extends StatefulWidget {
  const SummaryMissedScreen({super.key});

  @override
  State<SummaryMissedScreen> createState() => _SummaryMissedScreenState();
}

class _SummaryMissedScreenState extends BaseState<
    SummaryMissedScreen,
    SummaryMissedEvent,
    SummaryMissedState,
    SummaryMissedBloc> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      bloc.add(const SummaryMissedEvent.init());
    });
  }
    ...
    setState(() {
      ...
      if (_isSearchOpen) {
        ...
        if (_searchController.text.isNotEmpty) {
          ...
          bloc.add(const SummaryMissedEvent.changeKeyword(keyword: ''));
        }
      }
    });
  }
  ...
  void _showDateRangePicker(BuildContext context, SummaryMissedState state) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => DateRangePicker(
        ...
        onApply: (start, end) {
          bloc.add(SummaryMissedEvent.changeDateRange(
            dateStart: start,
            dateEnd: end,
          ));
        },
  ...
  void _showFilterSheet(BuildContext context, SummaryMissedState state) {
    showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (sheetContext) => SummaryMissedFilterSheet(
        departments: state.departments,
        selectedDepartmentId: state.departmentId,
        selectedStatus: state.filterStatus,
        onApply: (departmentId, status) {
          bloc.add(SummaryMissedEvent.changeFilters(
            departmentId: departmentId,
            status: status,
          ));
        },
        onClear: () {
          bloc.add(const SummaryMissedEvent.changeFilters(
            departmentId: null,
            status: null,
          ));
        },
  ...
  void _onSearchChanged(String value) {
    bloc.add(SummaryMissedEvent.changeKeyword(keyword: value));
  }

  void _onSearchSubmitted(String value) {
    bloc.add(SummaryMissedEvent.changeKeyword(keyword: value.trim()));
  }

  @override
  Widget renderUI(BuildContext context) {
    return BlocBuilder<SummaryMissedBloc, SummaryMissedState>(
  ...
  Widget _buildActiveFilters(SummaryMissedState state) {
  ...
  Map<String, List<SummaryMissedItem>> _groupByDepartment(
    List<SummaryMissedItem> items,
    List<SummaryMissedDepartment> departments,
  ) {
    final grouped = <String, List<SummaryMissedItem>>{};
    ...
    // Sort groups theo thứ tự departments list.
    final sorted = Map<String, List<SummaryMissedItem>>.fromEntries(
  ...
  int _countDistinctEmployees(List<SummaryMissedItem> items) {
    final seen = <int>{};
    for (final item in items) {
  ...
  Widget _buildContent(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              FilledButton(
                onPressed: () => bloc.add(const SummaryMissedEvent.refresh()),
    ...
    if (state.departmentId != null) {
      return RefreshIndicator(
        onRefresh: () async {
          bloc.add(const SummaryMissedEvent.refresh());
          await bloc.stream
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) => SummaryMissedCard(item: items[index]),
          ...
          return Column(
            ...
            children: [
              ...
              ...deptItems.map((item) => Padding(
                    ...
                    child: SummaryMissedCard(item: item),
                  )),
            ],
          );
        },
