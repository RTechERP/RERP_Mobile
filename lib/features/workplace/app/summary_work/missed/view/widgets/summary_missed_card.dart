...
/// Card hiển thị một phiếu quên chấm công trong danh sách summary.
class SummaryMissedCard extends StatelessWidget {
  const SummaryMissedCard({super.key, required this.item});

  final SummaryMissedItem item;

  static final _dateFmt = DateFormat('dd/MM/yyyy');

  /// Trạng thái TBP — null/false đều hiển thị "Chờ duyệt".
  ...
  /// Trạng thái HR — null/false đều hiển thị "Chờ duyệt".
  ApprovalStatus _hrStatus() {
    return item.isApprovedHR == true
        ? ApprovalStatus.approved
        : ApprovalStatus.pending;
  }
                ...
                Row(
                  children: [
                    Expanded(
                      ...
                      child: _TypeChip(
                        label: 'Loại',
                        value: item.typeText ?? 'Quên chấm công',
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (item.dayWork != null) ...[
                            const SizedBox(height: 8),
                            _InfoRow(
                              icon: Icons.calendar_today_outlined,
                              label: 'Ngày quên',
                              value: _formatDate(item.dayWork),
                            ),
                          ],
                          ...
                          if (item.approvedName != null &&
                              item.approvedName!.isNotEmpty) ...[
                            const SizedBox(height: 8),
                            _InfoRow(
                              icon: Icons.person_outline,
                              label: 'Người duyệt',
