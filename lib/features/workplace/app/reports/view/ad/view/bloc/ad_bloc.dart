...
@injectable
class AdBloc extends BaseBloc<AdEvent, AdState> {
  final ReportRepo _reportRepo;
  final LogUtils _log;
  final AuthRepo _authRepo;
  AdBloc(this._reportRepo, this._authRepo, this._log)
      : super(AdState.init()) {
  ...
  Future<void> _onInit(Emitter<AdState> emit) async {
  ...
  _onResetSubmitFlags(Emitter<AdState> emit) {
    emit(state.copyWith(submitSuccess: false));
    emit(state.copyWith(isSubmitting: false));
  }

  _onUpdateDate(DateTime? picked, Emitter<AdState> emit) {
  ...
  Future<void> _onSelectReport(int dailyID, Emitter<AdState> emit) async {
  ...
  Future<void> _onDeleteReport(int dailyID, Emitter<AdState> emit) async {
    emit(
      state.copyWith(
        isDeleting: true,
        deleteSuccess: false,
  ...
  Future<void> _onLoadDetailData(int dailyID, Emitter<AdState> emit) async {
    emit(state.copyWith(isLoadingDetail: true));
  ...
  _onResetCopy(Emitter<AdState> emit) {
    emit(state.copyWith(copyReports: [], copyError: null));
  }

  _onAddWork(Emitter<AdState> emit) {
  ...
  _onRemoveWork(int index, Emitter<AdState> emit) {
  ...
  _onExpandWork(int index, Emitter<AdState> emit) {
    emit(
      state.copyWith(
  ...
  Future<void> _onGetProject(Emitter<AdState> emit) async {
  ...
  _onSelectProject(ProjectResponse project, Emitter<AdState> emit) {
    emit(state.copyWith(selectedProject: project));
  }

  _onUpdateLocation(String type, String? value, Emitter<AdState> emit) {
    emit(
      state.copyWith(
        locationType: type,
  ...
  _onUpdatePlanNextDay(String planNextDay, Emitter<AdState> emit) {
