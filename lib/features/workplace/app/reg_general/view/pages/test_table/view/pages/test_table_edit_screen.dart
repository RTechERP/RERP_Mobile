// Date: 09/09/2026
// Nội dung/Chức năng: Màn chỉnh sửa phiếu đăng ký bàn test ESL.
//
// Flow:
//   1. Mở màn → initEdit(registrationId) → gọi /ESLRegistration/get-details
//      → populate form data từ response.
//   2. User chọn Dự án / Bàn test / Người đăng ký / Người duyệt / Ngày bắt đầu
//      → bloc.updateForm() cập nhật state.formData.
//   3. Bấm "Lưu" → submitRegistration() gọi /ESLRegistration/save
//      với payload chứa ID của phiếu cần cập nhật.
//   4. Submit thành công → submitSuccess = true → pop(true) để màn list refresh.
//
// Lookup data (project / testTable / employee / approver) được cache trong
// bloc.state và tái sử dụng giữa màn List và Edit.
...
class TestTableEditScreen extends StatefulWidget {
  /// Registration ID của phiếu cần chỉnh sửa.
  final int registrationId;

  /// Dữ liệu card từ màn list (để pre-populate form nếu có).
  final TestCardItem? cardItem;

  const TestTableEditScreen({
    super.key,
  ...
  @override
  State<TestTableEditScreen> createState() => _TestTableEditScreenState();
}

class _TestTableEditScreenState extends BaseShareState<TestTableEditScreen,
  ...
  void initState() {
    ...
    // Tải chi tiết phiếu card từ API.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      bloc.add(TestTableEvent.initEdit(
  ...
  void _onSave() {
    ...
    bloc.add(const TestTableEvent.editSubmit());
  }
  ...
  /// Xử lý feedback sau submit.
  void _handleFeedback(BuildContext context, TestTableState state) {
    // Submit thành công.
    if (state.submitSuccess) {
      showMessage(context, 'Cập nhật phiếu đăng ký bàn test thành công',
  ...
  Widget _buildForm(BuildContext context) {
    return BlocBuilder<TestTableBloc, TestTableState>(
      ...
      builder: (context, state) {
        final form = state.formData;
        final initialStartDate = form.startDate ?? _defaultStartDate();

        return BaseScaffold(
          backgroundColor: const Color(0xFFF4F7FB),
          appBar: AppBarCommon(
            title: const Text('Chỉnh sửa đăng ký bàn test'),
                ...
                child: BlocBuilder<TestTableBloc, TestTableState>(
                  ...
                  builder: (context, subState) {
                    return FormActions(
                      ...
                      onSave: subState.formData.isReady && !subState.isSubmitting
                          ? _onSave
                          : null,
                      saveText: 'Cập nhật',
                      cancelText: 'Huỷ',
